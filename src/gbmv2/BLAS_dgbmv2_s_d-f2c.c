...
extern void	FC_FUNC_(blas_dgbmv2_s_d, BLAS_DGBMV2_S_D)
                (int *trans, int *m, int *n, int *kl, int *ku, double *alpha, const float *a, int *lda, const double *head_x, const double *tail_x, int *incx, double *beta, double *y, int *incy)
{
  BLAS_dgbmv2_s_d(blas_colmajor, (enum blas_trans_type)*trans,
		  *m, *n, *kl, *ku, *alpha,
		  a, *lda, head_x,
		  tail_x, *incx, *beta,
		  y, *incy);
