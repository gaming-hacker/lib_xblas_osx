...
extern void	FC_FUNC_(blas_zgbmv2_z_c, BLAS_ZGBMV2_Z_C)
                (int *trans, int *m, int *n, int *kl, int *ku, const void *alpha, const void *a, int *lda, const void *head_x, const void *tail_x, int *incx, const void *beta, void *y, int *incy)
{
  BLAS_zgbmv2_z_c(blas_colmajor, (enum blas_trans_type)*trans,
		  *m, *n, *kl, *ku, alpha,
		  a, *lda, head_x,
		  tail_x, *incx, beta,
		  y, *incy);
