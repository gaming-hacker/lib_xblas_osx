 ...
 */
{
  ...
  case blas_prec_single:{
      ...
      int	      ky   , iy, kx, jx, j, i, rbound, lbound, ra, la, lenx,
      		      leny;
	...
	BLAS_error(routine_name, -14, incy, NULL);
      ...
      if ((m == 0) || (n == 0) ||
	  (((alpha_i[0] == 0.0 && alpha_i[1] == 0.0) && ((beta_i[0] == 1.0 && beta_i[1] == 0.0)))))
	return;
  ...
  case blas_prec_indigenous:
	...
	y_elem[1] = y_i[iy + 1];
	{
	  tmp2[0] = (double)beta_i[0] * y_elem[0] - (double)beta_i[1] * y_elem[1];
	  tmp2[1] = (double)beta_i[0] * y_elem[1] + (double)beta_i[1] * y_elem[0];
	}
	result[0] = tmp1[0] + tmp2[0];
	result[1] = tmp1[1] + tmp2[1];
	y_i[iy] = result[0];
