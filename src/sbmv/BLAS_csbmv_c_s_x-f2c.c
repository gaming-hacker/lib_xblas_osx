...
extern void	FC_FUNC_(blas_csbmv_c_s_x, BLAS_CSBMV_C_S_X)
                (int *uplo, int *n, int *k, const void *alpha, const void *a, int *lda, const float *x, int *incx, const void *beta, void *y, int *incy, int *prec)
{
  BLAS_csbmv_c_s_x(blas_colmajor, (enum blas_uplo_type)*uplo,
		   *n, *k, alpha, a, *lda,
		   x, *incx, beta,
