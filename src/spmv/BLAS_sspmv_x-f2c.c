...
extern void	FC_FUNC_(blas_sspmv_x, BLAS_SSPMV_X)
                (int *uplo, int *n, float *alpha, const float *ap, const float *x, int *incx, float *beta, float *y, int *incy, int *prec)
{
  BLAS_sspmv_x(blas_colmajor, (enum blas_uplo_type)*uplo,
	       *n, *alpha, ap,
	       x, *incx, *beta,
