...
extern void	FC_FUNC_(blas_zspmv_d_d_x, BLAS_ZSPMV_D_D_X)
                (int *uplo, int *n, const void *alpha, const double *ap, const double *x, int *incx, const void *beta, void *y, int *incy, int *prec)
{
  BLAS_zspmv_d_d_x(blas_colmajor, (enum blas_uplo_type)*uplo,
		   *n, alpha, ap,
		   x, *incx, beta,
