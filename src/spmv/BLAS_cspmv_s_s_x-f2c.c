...
extern void	FC_FUNC_(blas_cspmv_s_s_x, BLAS_CSPMV_S_S_X)
                (int *uplo, int *n, const void *alpha, const float *ap, const float *x, int *incx, const void *beta, void *y, int *incy, int *prec)
{
  BLAS_cspmv_s_s_x(blas_colmajor, (enum blas_uplo_type)*uplo,
		   *n, alpha, ap,
		   x, *incx, beta,
