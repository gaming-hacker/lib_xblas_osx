...
extern void	FC_FUNC_(blas_cspmv_s_c_x, BLAS_CSPMV_S_C_X)
                (int *uplo, int *n, const void *alpha, const float *ap, const void *x, int *incx, const void *beta, void *y, int *incy, int *prec)
{
  BLAS_cspmv_s_c_x(blas_colmajor, (enum blas_uplo_type)*uplo,
		   *n, alpha, ap,
		   x, *incx, beta,
