...
extern void	FC_FUNC_(blas_csymv_s_c_x, BLAS_CSYMV_S_C_X)
                (int *uplo, int *n, const void *alpha, const float *a, int *lda, const void *x, int *incx, const void *beta, void *y, int *incy, int *prec)
{
  BLAS_csymv_s_c_x(blas_colmajor, (enum blas_uplo_type)*uplo,
		   *n, alpha, a, *lda,
		   x, *incx, beta,
