...
extern void	FC_FUNC_(blas_ssymv_x, BLAS_SSYMV_X)
                (int *uplo, int *n, float *alpha, const float *a, int *lda, const float *x, int *incx, float *beta, float *y, int *incy, int *prec)
{
  BLAS_ssymv_x(blas_colmajor, (enum blas_uplo_type)*uplo,
	       *n, *alpha, a, *lda,
	       x, *incx, *beta,
