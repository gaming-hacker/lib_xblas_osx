...
extern void	FC_FUNC_(blas_zsymv_d_z_x, BLAS_ZSYMV_D_Z_X)
                (int *uplo, int *n, const void *alpha, const double *a, int *lda, const void *x, int *incx, const void *beta, void *y, int *incy, int *prec)
{
  BLAS_zsymv_d_z_x(blas_colmajor, (enum blas_uplo_type)*uplo,
		   *n, alpha, a, *lda,
		   x, *incx, beta,
