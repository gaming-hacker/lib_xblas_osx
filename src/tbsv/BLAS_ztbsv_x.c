  ...
  case blas_prec_double:{
...

	{
	  ...
	  if ((trans == blas_conj) || (trans == blas_conj_trans)) {
		...
		x_elem[1] = x_i[xi + 1];
		{
		  temp2[0] = (double)x_elem[0] * T_element[0] - (double)x_elem[1] * T_element[1];
		  temp2[1] = (double)x_elem[0] * T_element[1] + (double)x_elem[1] * T_element[0];
		}
  ...
  case blas_prec_extra:{
...

	{
	  ...
	  double	  head_temp3[2], tail_temp3[2];	/* temporary variable
							 * for calculations */
	  double	  x_elem [2];
	  double	  T_element[2];	/* temporary variable for an element
					 * of matrix T */
