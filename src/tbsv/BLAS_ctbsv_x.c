 ...
 */
{
  ...
  if (k == 0) {
    if (((alpha_i[0] == 1.0 && alpha_i[1] == 0.0)) && (diag == blas_unit_diag)) {
      /* nothing to do */
      return;
    } else {
  ...
  case blas_prec_single:{
	...
	{
	  float		  temp1  [2];	/* temporary variable for
					 * calculations */
	  float		  temp2  [2];	/* temporary variable for
					 * calculations */
	  float		  x_elem [2];
	  float		  T_element[2];
	  ...
	  if ((trans == blas_conj) || (trans == blas_conj_trans)) {
	    ...
	    for (j = 0; j < k; j++) {
	      ...
	      x_elem[1] = x_i[xi + 1];
	      /*
	       * preform the multiplication - in this implementation we do
	       * not seperate the alpha = 1 case
	       */
	      {
  ...
  case blas_prec_double:{
	...
	{
	  double	  temp1  [2];	/* temporary variable for
					 * calculations */
	  double	  temp2  [2];	/* temporary variable for
					 * calculations */
	  double	  temp3  [2];	/* temporary variable for
					 * calculations */
	  float		  x_elem [2];
	  float		  T_element[2];	/* temporary variable for an element
					 * of matrix T */

	  int		  x_inti = 0, inc_x_inti = 1;
	  int		  k_compare = k;	/* used for comparisons with
						 * x_inti */
	  double         *x_internal;
	  ...
	  if ((trans == blas_conj) || (trans == blas_conj_trans)) {
	    ...
	    for (j = 0; j < k; j++) {
	       ...
	       */
	      {
		temp1[0] = (double)x_elem[0] * alpha_i[0] - (double)x_elem[1] * alpha_i[1];
		temp1[1] = (double)x_elem[0] * alpha_i[1] + (double)x_elem[1] * alpha_i[0];
	      }
		...
		{
		  temp2[0] = (double)temp3[0] * T_element[0] - (double)temp3[1] * T_element[1];
		  temp2[1] = (double)temp3[0] * T_element[1] + (double)temp3[1] * T_element[0];
		}
		temp1[0] = temp1[0] - temp2[0];
		temp1[1] = temp1[1] - temp2[1];
		x_inti += inc_x_inti;
  ...
  case blas_prec_extra:{
	...
	{
	  double	  head_temp1[2], tail_temp1[2];	/* temporary variable
							 * for calculations */
	  double	  head_temp2[2], tail_temp2[2];	/* temporary variable
							 * for calculations */
	  double	  head_temp3[2], tail_temp3[2];	/* temporary variable
							 * for calculations */
	  float		  x_elem [2];
	  float		  T_element[2];	/* temporary variable for an element
					 * of matrix T */
	  ...
	  if ((trans == blas_conj) || (trans == blas_conj_trans)) {
...

		{
		  ...
		  if (ab > ov1 / 16) {	/* scale down a, b */
			 ...
			 */
			double		b1     , b2, con, e, t1, t2, t11, t21,
					t12          , t22;
			...
			/*
			 * Split t1 and b into two parts with at most 26 bits
			 * each, using the Dekker-Veltkamp method.
			 */
			con = t1 * split;
			t11 = con - (con - t1);
			t21 = t1 - t11;
			...
			/*
			 * Compute high-order word of (t11, t21) and divide
			 * by b.
			 */
			t2 = (t11 + t21) / 16.0;
		  ...
		  /*
		   * Now un1/eps1*2 <= (a,b) >= ov1/16, un/eps*2 <= (c,d) >=
		   * ov/16
		   */
		  if (abs_c > abs_d) {
		    r = T_element[1] / T_element[0];
