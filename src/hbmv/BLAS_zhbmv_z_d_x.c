 ...
 */
{
  ...
  case blas_prec_double:{
	...
	/*
	 * maj_first is number of elements done in first loop (this will
	 * increase by one over each column up to a limit)
	 */
	maxj_first = 0;

	/*
	 * maxj_second is number of elements done in second loop the first
	 * time
	 */
	...
	if (uplo == blas_lower) {
	  ...
	  if ((alpha_i[0] == 1.0 && alpha_i[1] == 0.0)) {
		...
		if (i >= k) {
	      ...
	      /*
	       * Case alpha = 1, but beta != 0. We compute  y  <--- A * x +
	       * beta * y
	       */
	      for (i = 0, yi = y_starti; i < n_i; i++, yi += incy) {
		sum[0] = sum[1] = 0.0;
	...
	if (uplo == blas_lower) {
	  ...
	  if ((alpha_i[0] == 1.0 && alpha_i[1] == 0.0)) {
		...
		x_elem = x_i[xi];
		{
		  ...
		  tail_prod[0] = (((a1 * b1 - head_prod[0]) + a1 * b2) + a2 * b1) + a2 * b2;
