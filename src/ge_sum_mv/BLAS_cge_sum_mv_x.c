  ...
  case blas_prec_double:
	...
	if (beta_i[0] == 0.0 && beta_i[1] == 0.0) {
	  ...
	  for (i = 0, yi = y_starti; i < m; i++, yi += incyi) {
	    ...
	    for (j = 0, xi = x_starti; j < n; j++, xi += incxi) {
	      ...
	      b_elem[1] = b_i[bij + 1];
	      {
		prod[0] = (double)b_elem[0] * x_elem[0] - (double)b_elem[1] * x_elem[1];
		prod[1] = (double)b_elem[0] * x_elem[1] + (double)b_elem[1] * x_elem[0];
	      }
	      sumB[0] = sumB[0] + prod[0];
	      sumB[1] = sumB[1] + prod[1];
	      bij += incbij;
	  ...
	  for (i = 0, yi = y_starti; i < m; i++, yi += incyi) {
	    ...
	    /* now put the result into y_i */
	    {
	      tmp1[0] = (double)sumB[0] * beta_i[0] - (double)sumB[1] * beta_i[1];
	      tmp1[1] = (double)sumB[0] * beta_i[1] + (double)sumB[1] * beta_i[0];
	    }
	    y_i[yi] = tmp1[0];
	    y_i[yi + 1] = tmp1[1];
	...
	if (beta_i[0] == 0.0 && beta_i[1] == 0.0) {
	  ...
	  for (i = 0, yi = y_starti; i < m; i++, yi += incyi) {
	    ...
	    tmp1[1] = sumA[1];
	    {
	      tmp2[0] = (double)sumB[0] * beta_i[0] - (double)sumB[1] * beta_i[1];
	      tmp2[1] = (double)sumB[0] * beta_i[1] + (double)sumB[1] * beta_i[0];
	    }
	    tmp1[0] = tmp1[0] + tmp2[0];
	    tmp1[1] = tmp1[1] + tmp2[1];
	    y_i[yi] = tmp1[0];
	...
	if (beta_i[0] == 0.0 && beta_i[1] == 0.0) {
	  ...
	  for (i = 0, yi = y_starti; i < m; i++, yi += incyi) {
	    ...
	    /* now put the result into y_i */
	    {
	      tmp1[0] = (double)sumA[0] * alpha_i[0] - (double)sumA[1] * alpha_i[1];
	      tmp1[1] = (double)sumA[0] * alpha_i[1] + (double)sumA[1] * alpha_i[0];
	    }
	    y_i[yi] = tmp1[0];
	    y_i[yi + 1] = tmp1[1];
	    ai += incai;
