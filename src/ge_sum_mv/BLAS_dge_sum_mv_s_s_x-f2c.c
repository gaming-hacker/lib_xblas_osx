...
extern void	FC_FUNC_(blas_dge_sum_mv_s_s_x, BLAS_DGE_SUM_MV_S_S_X)
                (int *m, int *n, double *alpha, const float *a, int *lda, const float *x, int *incx, double *beta, const float *b, int *ldb, double *y, int *incy, int *prec)
{
  BLAS_dge_sum_mv_s_s_x(blas_colmajor, *m, *n,
			*alpha, a, *lda,
			x, *incx,
			*beta, b, *ldb,
